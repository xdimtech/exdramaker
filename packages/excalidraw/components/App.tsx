...
class App extends React.Component<AppProps, AppState> {
                  ...
                  <ExcalidrawAppStateContext.Provider value={this.state}>
                    ...
                    >
                      ...
                      >
                        ...
                        />
                        <SlideNavigator zoom={this.state.zoom.value} />
  ...
  private resetScene = withBatchedUpdates(
    (opts?: { resetLoadingState: boolean }) => {
      ...
      this.resetStore();
      this.resetHistory();
      // Also clear slides when resetting scene
      editorJotaiStore.set(slidesAtom, []);
    },
  );
  ...
  public async componentDidMount() {
    ...
    // Listen for slide scroll events
    const handleSlideScroll = (e: Event) => {
      ...
      if (
        ...
        // Calculate viewport position for the target scene coordinates
        const zoom = this.state.zoom;
        // Calculate where to scroll to center the target
        const targetScrollX = detail.x * zoom.value - this.state.width / 2;
        const targetScrollY = detail.y * zoom.value - this.state.height / 2;
    ...
    window.addEventListener("excalidraw-scroll-to", handleSlideScroll);

    // Listen for zoom change events from SlideNavigator
    const handleRecordingZoom = (e: Event) => {
      const detail = (e as CustomEvent).detail;
      if (detail && typeof detail.zoom === "number") {
        this.setState({
          zoom: { value: detail.zoom },
        });
      }
    };
    window.addEventListener("excalidraw-recording-zoom", handleRecordingZoom);

    // Handle recording setup - set both zoom and scroll at once
    const handleRecordingSetup = (e: Event) => {
      const detail = (e as CustomEvent).detail;
      if (detail) {
        const updates: any = {};
        if (typeof detail.zoom === "number") {
          updates.zoom = { value: detail.zoom };
        }
        if (typeof detail.scrollX === "number") {
          updates.scrollX = detail.scrollX;
        }
        if (typeof detail.scrollY === "number") {
          updates.scrollY = detail.scrollY;
        }
    ...
    window.addEventListener("excalidraw-recording-setup", handleRecordingSetup);
  ...
  setActiveTool = (
    ...
    // When switching to a non-selection tool, deselect the active slide
    // This allows drawing elements inside slides
    if (!isSelectionLikeTool(nextActiveTool.type)) {
      editorJotaiStore.set(activeSlideIdAtom, null);
    }
  ...
  private handleCanvasPointerDown = (
    ...
    // Deselect slide when clicking on canvas (not on a slide)
    // This allows drawing elements inside slides
    editorJotaiStore.set(activeSlideIdAtom, null);
  ...
  loadFileToCanvas = async (
    ...
    try {
      ...
      if (ret.type === MIME_TYPES.excalidraw) {
        ...
        // Restore slides from imported data
        if (ret.data.slides && ret.data.slides.length > 0) {
          editorJotaiStore.set(slidesAtom, ret.data.slides);
          if (ret.data.slides.length > 0) {
            this.setState({});
          }
        }
